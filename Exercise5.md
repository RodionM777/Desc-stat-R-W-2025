...
```python
df_pokemon['Defense'].hist(bins=20, edgecolor='black')
plt.xlabel('Defense Score')
plt.ylabel('Frequency')
plt.title('Distribution of Pokémon Defense Scores')
...
type_counts.plot.bar(figsize=(12, 6), color='skyblue', edgecolor='black')

plt.xlabel('Pokémon Type', fontsize=12)
plt.ylabel('Count', fontsize=12)
plt.title('Frequency of Pokémon Types (Type 1)', fontsize=14)
...
```
    ...
    <matplotlib.patches.FancyArrow at 0x2c75d76f750>
...
I assume the distribution of data is rather normal with an outlier on the rigth side , maybe a little right skewed because of theseoutliers
...
```
    ...
    9.966674912678634 9.991103338303429
    

the mean>median so yes a little scewed to the right, but not a lot
...
```
    ...
    <matplotlib.lines.Line2D at 0x2c75d7c6990>
...
```python
slope, intercept = np.polyfit(df_pokemon['Attack'], df_pokemon['Speed'], 1)

plt.figure(figsize=(10, 6))

# Scatter plot with transparency
plt.scatter(x='Attack', y='Speed', data=df_pokemon, alpha=0.5)

# Add regression line
plt.plot(df_pokemon['Attack'], slope*df_pokemon['Attack'] + intercept, color='red')

# Labels and title
plt.xlabel('Attack', fontsize=12)
plt.ylabel('Speed', fontsize=12)
plt.title('Relationship Between Pokémon Attack and Speed', fontsize=14)

# Calculate and annotate correlation
correlation = df_pokemon[['Attack', 'Speed']].corr().iloc[0,1]
plt.annotate(f'Pearson r = {correlation:.2f}', xy=(0.7, 0.9), xycoords='axes fraction')
...
![png](Exercise5_files/Exercise5_68_0.png)
